...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => true
      case y::Nil => true
      case y::ys => (y == ys.head) && allEqual(ys)
    }
  ...
  def stringLengths(xs: List[String]): List[(String, Int)] =
    // TODO: Provide definition here.
    // throw UnsupportedOperationException()
    if (xs == Nil) then List() else xs.map(xs => (xs, xs.length))
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    ...
    xs match{
	    case Nil => Nil
	    case y::Nil => List(y)
	    case y::ys => if (y == ys.head) then removeDupes1(ys) else y::removeDupes1(ys)
	  }
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    (n, xs) match{
      ...
      if(n > 0) then (x::lista, listb) else (lista, x::listb)}
    }
  ...
  def allDistinct(xs: List[Int]): Boolean =
    ...
    xs match {
      ...
      case x::xs => if (member(x, xs)) then return false else allDistinct(xs)
    }
